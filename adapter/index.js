...
module.exports = async function adapter(builder) {
  ...
  const publish = resolve(__dirname, "..", netlify_config.build.publish);
  const functions = resolve(__dirname, "..", netlify_config.build.functions);
  ...
  // copy the renderer
  copyFileSync(
    resolve(__dirname, "files/render.js"),
